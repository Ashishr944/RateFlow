...
router.use((0, auth_1.requireRole)([client_1.Role.USER]));
...
router.post('/ratings', async (req, res) => {
    try {
        ...
        // Check store exists
        const storeExists = await db_1.default.store.findUnique({ where: { id: storeId } });
        if (!storeExists) {
...
router.put('/ratings/:id', async (req, res) => {
    try {
        ...
        const dbRating = await db_1.default.rating.findUnique({ where: { id } });
        if (!dbRating) {
